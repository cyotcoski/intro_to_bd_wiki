...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def f(acc: Int, article: WikipediaArticle):Int =
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    langs
      .map(l => (l, occurrencesOfLang(l,rdd))).sortBy(_._2)
      .reverse
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    ...
    val pairs: RDD[(String, WikipediaArticle)] = rdd.flatMap(article => langs.map(l =>(l, article)))

    // 2. Second, filter the `pairs` RDD to create a collection of all 
    //    pairs (l, wa) where wa is an article that mentions language l.
    val mentionedPairs: RDD[(String, WikipediaArticle)] = rdd.flatMap(articles => langs.filter(l => articles.mentionsLanguage(l))
      .map(langAndArticle => (langAndArticle, articles)))
    ...
    mentionedPairs.groupByKey()// <<<<  replace ??? with what you want the `makeIndex` function to return.
  }
  // Computes the language ranking using the inverted index.
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = 
    index.mapValues(temp=> temp.size).sortBy(_._2).collect().toList.reverse
  ...
  def zipLangWithPoint(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Int)] =
    rdd.flatMap(zippedArticles=>langs.filter(l => zippedArticles.mentionsLanguage(l))).map(ln =>(ln,1))
  /** Uses `reduceByKey` to compute the index and the ranking simultaneously.
    */
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = 
    zipLangWithPoint(langs,rdd).reduceByKey(_+_).sortBy(_._2).collect().toList.reverse
